...
const gameEvents = {
  ...
  roundChange: () => {
    ...
    socket.broadcast.to(room).emit('resetButtons', {reset: 'buttons'});
    ...
    knex('questions').where({id: gameEvents.roundLeft}).then(res => {
      if (gameEvents.roundLeft > 0) {
        ...
        socket.broadcast.to(room).emit('roundChange', { roundNumber: gameEvents.roundLeft, gameQuestion: gameEvents.gameQuestion, gameAnswers: gameEvents.gameAnswers });

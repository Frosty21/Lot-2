...
const gameEvents = {
  ...
  setPoints: (username) => {
    ...
    if (gameEvents.usersScores[username]){
      gameEvents.usersScores[username] = gameEvents.usersScores[username] + 125;
    } else {
  ...
  roundChange: () => {
    ...
    io.to(room).emit('resetButtons', {reset: 'buttons'});
    ...
    knex('questions').where({id: gameEvents.roundLeft}).then(res => {
      if (gameEvents.roundLeft > 0) {
        ...
        io.to(room).emit('roundChange', { roundNumber: gameEvents.roundLeft, gameQuestion: gameEvents.gameQuestion, gameAnswers: gameEvents.gameAnswers });
        gameEvents.roundLeft = gameEvents.roundLeft - 1;
	gameEvents.Round = gameEvents.Round + 1;
  ...
  gameStart: (socket, room) => {
    ...
    gameEvents.roundLeft = 6;
    gameEvents.roundTimer = 15000;
    gameEvents.roundInterval = gameEvents.roundTimer / 1000;
    gameEvents.socket = socket;
    gameEvents.room = room;
    socket.broadcast.to(gameEvents.room).emit('gameStarted', {gameId: 'x01'});
    gameEvents.roundChange();
  },

  gameEnded: () => {
    const io = gameEvents.io,
          room = gameEvents.room;
    io.to(room).emit('gameEnded', { usersScores: gameEvents.usersScores });
  },

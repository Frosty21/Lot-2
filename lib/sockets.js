  ...
  roundChange: () => {
    ...
    knex('questions').where({id: gameEvents.roundLeft}).then(res => {
      if (gameEvents.roundLeft > 0) {
        ...
        socket.to(room).emit('roundChange', { roundNumber: gameEvents.roundLeft, gameQuestion: gameEvents.gameQuestion, gameAnswers: gameEvents.gameAnswers , correctAnswer: gameEvents.correctAnswer});

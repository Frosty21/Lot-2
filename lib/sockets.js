...
const gameEvents = {
  ...
  roundChange: () => {
    ...
    socket.volatile.emit('resetButtons');
    ...
    knex('questions').where({id: gameEvents.roundLeft}).then(res => {
      if (gameEvents.roundLeft > 0) {
        ...
        socket.volatile.to(room).emit('roundChange', { roundNumber: gameEvents.roundLeft, gameQuestion: gameEvents.gameQuestion, gameAnswers: gameEvents.gameAnswers });
